...
MTFHashTable::MTFHashTable(int k): H(10007, 0), visited(10007, false), k(k) {}

void MTFHashTable::print_stats() const {
    std::cout << "Used hash cells = " << used_cells << std::endl;
    std::cout << "Hash table load = " << used_cells / double(H.size()) << std::endl;
}
...
void MTFHashTable::encode(const uint8_t *block, long size, uint32_t *out_block) {
    ...
    while (i < size) {
        ...
        uint32_t pos = mtfEncode(H[hash.get_hash()], c);
        out_block[i] = pos;
